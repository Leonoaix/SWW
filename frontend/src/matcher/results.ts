...
function applyAction(job: RankedJob): HTMLElement {
  ...
  const refresh = (note = "") => {
    state.textContent = note || (hasOpened(job.id) ? "已打开过申请页" : "");
    state.hidden = !state.textContent;
  };

  if (!addressable && !job.id) {
    row.append(element("span", "field-hint", "未获得职位链接，也没有编号，无法定位该职位。"));
    return row;
  }

  const ask = (control: HTMLElement) => {
    markOpened(job.id);
    control.setAttribute("aria-busy", "true");
    refresh("正在登录浏览器中打开…");
    void post(`/jobs/${encodeURIComponent(job.id)}/open`)
      .then(() => refresh())
      .catch((error: Error) => refresh(error.message))
      .finally(() => control.removeAttribute("aria-busy"));
  };

  if (addressable) {
    const link = externalLink(url as string, "去申请 ↗", "btn btn-primary btn-sm");
    link.addEventListener("click", event => {
      if (!loginBrowserOpen) { markOpened(job.id); refresh(); return; }
      event.preventDefault();          // the logged-in browser already has the session
      ask(link);
    });
    row.append(link);
    row.append(state);
  ...
  // Most postings have no address: the board opens their detail as a dialog
  // from an onclick handler. A link would land on the board instead, so this
  // asks the service to do what the crawler does — find the row and click it.
  const button = element("button", "btn btn-primary btn-sm", "去申请 ↗");
  button.type = "button";
  button.addEventListener("click", () => {
    if (!loginBrowserOpen) {
      refresh("此职位的详情是看板上的弹窗，请先点「打开登录浏览器」。");
      return;
    }
    ask(button);
  });

  // Keep the manual route open: the id is what the board's search box takes.
